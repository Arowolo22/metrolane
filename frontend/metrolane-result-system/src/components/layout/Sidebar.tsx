...
export function Sidebar({
  ...
  return (
    <>
      {/* Mobile backdrop */}
      <div
        className={`fixed inset-0 z-30 bg-black/40 transition-opacity lg:hidden ${isOpen ? "block" : "hidden"}`}
        onClick={onClose}
        aria-hidden={!isOpen}
      />

      <aside
        className={`fixed inset-y-0 left-0 z-40 w-64 flex-col border-r border-gray-200 bg-white lg:flex ${isOpen ? "flex" : "hidden"}`}
      >
