...
export async function generateResultPdf({
  ...
  try {
    ...
    const pairedStudentRows = studentRows.reduce<Array<[string, string, string, string]>>(
      (rows, current, index) => {
        if (index % 2 === 0) {
    ...
    summaryItems.forEach((item, index) => {
      ...
      drawCompactMetricCard(doc, x, y, 88, 13, item[0], item[1], undefined, false)
    })
    ...
    cards.forEach((card, index) => {
      ...
      const y = cursorY
      drawCompactMetricCard(doc, x, y, cardWidth, cardHeight, card.title, card.value, card.sub, card.fill === ORANGE)
    })
    ...
    signatureLabels.forEach((label, index) => {
      ...
      doc.setFontSize(7.2)
      doc.setTextColor(...DARK_GRAY)
      doc.text(label, x + signatureWidth / 2, cursorY + 16, { align: "center" })
    })
    ...
    doc.setFont("helvetica", "normal")
    doc.setFontSize(9)
    doc.text("The system generated a simplified fallback page instead.", 14, 50)
