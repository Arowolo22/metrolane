...
const STATUS_COPY: Record<ApiErrorKind, ErrorStatusCopy> = {
  network: {
    ...
    description: "Check your internet connection. We’ll retry when you’re back online.",
    actionLabel: "Try again",
  },
  server: {
    title: "The service is unavailable",
    description: "We couldn’t load this view right now. Please try again in a moment.",
    actionLabel: "Retry",
  },
  badRequest: {
  ...
  unauthorized: {
    ...
    description: "Sign in again to continue. Your saved work will remain protected.",
  ...
  conflict: {
    ...
    description: "Use a different session or update the existing record instead.",
...
function isRetryableKind(kind: ApiErrorKind): boolean {
  return kind === "network" || kind === "server" || kind === "rateLimited" || kind === "unknown"
}
...
export function classifyApiError(
  ...
  const kind = isOfflineNetworkError(error) ? "network" : kindFromStatus(status)
  ...
  return new ResilienceError({
    ...
    message: kind === "unknown" ? fallback : copy.description,
    cause: error,
  })
}
